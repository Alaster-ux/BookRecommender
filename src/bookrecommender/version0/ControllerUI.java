...
/**
 * Controller principale per tutte le FXML dell'applicazione.
 * Gestisce login, registrazione, ricerca libri, librerie e interazioni col server.
 */
public class ControllerUI implements Initializable {
	...
	private ControllerUI previousController;
	...
	public void setListController(LinkedList<Stage> ctrl) {
		if(ctrl==null) {
			ctrl = new LinkedList<Stage>();
		}
		this.stageList = ctrl;
	}
	...
	private void setPreviousController(ControllerUI controller) {
	...
	public void openFXML(String fxml) throws IOException {
		...
		if(fxml.equals(Home)) {
		 ...
		 controller.setListController(this.stageList);
         Stage stage = new Stage();
         if(!fxml.equals(Home)) {
        	 controller.stageList.add(stage);
         }
	...
	private String getPreviousFXML() {
		...
		if(currentFXML.equals(Libreria)) {
	...
	public void confermaSceltaLibro(ActionEvent event) throws IOException {
		...
		if(index<0) {
			...
			if(nextFXML.equals(Valutazione)) {
				...
				this.closeFXML(event);
				this.previousController.initializeInfoLibro();
				if(check==0) {
			...
			if(nextFXML.equals(Libreria)) {
				...
				if(check==0) {
					...
					this.closeFXML(event);
					this.previousController.inizializzaLibreria();
				}else {
	...
	public void cercaAutoreAnno(ActionEvent event) throws IOException {
		String autore = this.CercaLibro_Author.getText().trim();
		if(autore.equals("")) {
			warning("Errore","Inserire un Autore.");
			return;
		}
	...
	private void initializeInfoLibro() {
		...
		System.out.println("infoLibro -> "+this.currentLibro);
	...
	public void ConfermaCreaLibreria(ActionEvent event) {
	   ...
	   version0.Libreria check = this.currentProxy.registraLibreria(this.currentUserID, nomeLib);
	   if(check!=null) {
		   warning("Notifica","La libreria è stata creata con successo");
		   this.closeFXML(event);
		   this.previousController.initializeLibrerieUtente();

	   }else {
		   warning("Errore","La libreria non è stata creata.");
	   }
	}
